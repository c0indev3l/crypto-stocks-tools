...
class API_Request:
    def __init__(self, base_path, api_url, values={}, api_type='json', flag_download=True):
        ...
        self.api_base_url = "https://www.havelockinvestments.com/r/"
    ...
    def download(self):
        ...
        req = requests.post(self.url, data=self.values, verify=False)
        # verify=False => to fix requests.exceptions.SSLError: [Errno 1] _ssl.c:504: error:14090086:SSL routines:SSL3_GET_SERVER_CERTIFICATE:certificate verify failed
...
class API_Request_get_dividend_for_security(API_Request):
    ...
    def convert_to_DataFrame(self):
        if self.data['status'].lower()=='ok':
            try:
                if len(self.data['dividends'])>0:
                    ...
                    self.df = pd.DataFrame(columns=['dividend_per_share', 'announcedate', 'timestamp'])
            except:
        ...
        self.df['announcedate'] = pd.to_datetime(self.df['announcedate'])
...
class API_Request_get_list_of_securities(API_Request):
    def __init__(self, base_path, api_type='json', flag_download=True):
        ...
        self.df = self.df.set_index('ticker', verify_integrity=True)
        ...
        self.df['DividendPerPricePC'] = self.df['dividend_per_share']/self.df['last'] * 100.0
        self.df['DividendPerPricePC'] = self.df['DividendPerPricePC'].fillna(0.0)

        # URL
        #self.df['url'] = self.df.index
        #self.df['url'] = 'https://.....com/.../' + self.df['url']
        ...
        #df = self.df[['name', 'last', 'dividends_nb', 'dividend_per_share']]
        df = self.df[['name', 'last', 'dividends_nb', 'dividend_per_share', 'DividendPerPricePC']]
        #df = self.df[['name', 'last', 'dividends_nb', 'dividend_per_share', 'SpreadRelPC', 'DividendPerPricePC']]
        print(df)
    ...
    def convert_to_DataFrame(self):
        data = [val for key, val in self.data.iteritems()]
        self.df = pd.DataFrame(data)
        
        cols = ['1d', '30d', '7d']
        for col in cols:
            for i in self.df.index:
                for key, value in self.df[col][i].iteritems():
                    newcol = col + '_' + key
                    if newcol not in self.df.columns:
                        self.df[newcol] = np.nan
                    self.df[newcol][i] = value
        for col in cols:
            del self.df[col]
    ...
    def calculate(self):
        self.df['last'] = self.df['last'].astype(float)
        self.df['units'] = self.df['units'].astype(int)

    def tickers(self):
        for ticker in self.df.index: #['ticker']:
                yield ticker
