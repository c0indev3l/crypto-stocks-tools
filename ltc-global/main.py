...
class API_Request:
  def __init__(self, base_path, api_url, values={}, api_type='json', flag_download=True):
    ...
    self.api_base_url = "https://www.litecoinglobal.com/api/"
...
class API_Request_get_list_of_securities(API_Request):
  def __init__(self, base_path, api_type='json', flag_download=True):
    ...
    # Sort
    self.df = self.df.sort('DividendPerPricePC', ascending=False)
    
    print(self.df[['ask', 'bid', 'dividends_nb', 'dividend_per_share', 'DividendPerPricePC', 'type']])


    #filename = os.path.join(self.base_path, "data_out/data.csv")
    #self.df.to_csv(filename)
    #filename = os.path.join(self.base_path, "data_out/_tickers.xls")
    #self.df.to_excel(filename)
